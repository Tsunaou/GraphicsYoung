...
**数值差分分析DDA(digital differential analyzer)：**直接利用$\Delta x$或$\Delta y$的线段扫描转换算法，利用光栅特性(屏幕单位网格表示像素列阵)，使用$x$或$y$方向单位增量(用$\Delta x$或$\Delta y=±1$)来离散取样，并逐步计算沿线路径各像素位置。在一个坐标轴上以单位间隔对线段离散取样，确定另一个坐标轴上最靠近线段路径的对应整数值。
